...
export default function Footer() {
  ...
  return (
    <footer className="bg-bg-deep border-t border-border-sub pt-12 pb-24 lg:pb-6 px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-6 lg:gap-10 mb-10">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1 text-[14px] text-text-2">
              <span>{dealer.workday.day}</span>
              <span>{dealer.workday.hours}</span>
            </div>
          </div>
        </div>

        <div className="border-t border-border-sub pt-5 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1">
            {SOCIAL_LINKS.map(({ label, href, icon }) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={label}
                className="w-9 h-9 flex items-center justify-center rounded-sm text-text-3 hover:text-text-1 hover:bg-bg-card transition-colors duration-200"
              >
                {icon}
              </a>
            ))}
